...
const Main = () => {
  ...
  const findNodeDetails = (nodeId, graphData) => {
    ...
    // Obtain the links from the graph data
    const links = [
    ...
    // Filter the links based on the node ID
    ...
    const used = usedLinks.map((link) => link.target.id).join(", ") || "None";
